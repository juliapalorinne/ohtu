Feature: A new user account can be created if a proper unused username and password are given
    ...
    Scenario: creation fails with too short username and valid password
        Given command new user is selected
        When  a valid username "jp" and password "salainen1" and matching password confirmation are entered
        Then  user is not created and too short username error is reported

    Scenario: creation fails with correct username and too short password
        Given command new user is selected
        When  a valid username "lissu" and too short password "sala1" and matching password confirmation are entered
        Then  user is not created and too short password error is reported

    Scenario: creation fails when password and password confirmation do not match
        Given command new user is selected
        When  a valid username "erja" and password "salainen1" and non-matching password confirmation "salainen2" are entered
